...
export class AuthClient {
    ...
    login(credential: string | null | undefined): Observable<FileResponse> {
        let url_ = this.baseUrl + "/api/Auth/Login";
        url_ = url_.replace(/[?&]$/, "");

        const content_ = new FormData();
        if (credential !== null && credential !== undefined)
            content_.append("credential", credential.toString());
    ...
    getCurrent(): Observable<GetCurrentResponse> {
        let url_ = this.baseUrl + "/api/Auth/GetCurrent";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<GetCurrentResponse>;
                }
            } else
                return _observableThrow(response_) as any as Observable<GetCurrentResponse>;
        }));
    }

    protected processGetCurrent(response: HttpResponseBase): Observable<GetCurrentResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = GetCurrentResponse.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap((_responseText: string) => {
...
export class GetCurrentResponse implements IGetCurrentResponse {
    user?: UserResponse | undefined;

    constructor(data?: IGetCurrentResponse) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): GetCurrentResponse {
        data = typeof data === 'object' ? data : {};
        let result = new GetCurrentResponse();
        result.init(data);
        return result;
    }
