...
using MoneySpot6.WebApp.Features.Ui.Stocks.PriceImport;

namespace MoneySpot6.WebApp.Features.Core.SampleData;

[ScopedService]
public class SampleDataSeeder(Db db, AccountSyncService accountSyncService, StockUpdater stockUpdater)
{
    public async Task Seed()
    {
        ...
        await accountSyncService.SyncAll(new NoOpCallbackHandler(), CancellationToken.None);

        await db.BankAccountTransactions
            .Where(t => t.IsNew)
            .ExecuteUpdateAsync(s => s.SetProperty(t => t.IsNew, false));

        await stockUpdater.Update(CancellationToken.None);
        return;
    ...
    private class NoOpCallbackHandler : IAdapterCallbackHandler
    {
        public Task<string?> OnTanRequired(string message, CancellationToken ct) =>
            throw new InvalidOperationException("Sample data sync should not request a TAN.");

        public Task<string?> OnSecurityMechanismRequired(ImmutableDictionary<string, string> securityMechanism, CancellationToken ct) =>
            throw new InvalidOperationException("Sample data sync should not request a security mechanism.");

        public Task OnLogMessage(int severity, string message, CancellationToken ct) => Task.CompletedTask;
    }
}
